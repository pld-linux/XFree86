...
Summary(pl):	Pliki XFree86 wymagane zarСwno po stronie serwera jak i klienta
Group:		X11/XFree86
...
Pliki XFree86 wymagane zarСwno po stronie serwera jak i klienta.
...
Summary(pl):	Biblioteka do renderowania fontСw
Group:		X11/XFree86
...
Xft jest bibliotek╠ sЁu©╠c╠ do renderowania fontСw dla X Window.

%package xft-devel
Summary:	Header files for X Font Rendering library
Summary(pl):	Pliki nagЁСwkowe dla biblioteki do renderowania fontСw
...
Ten pakiet zawiera pliki nagЁСwkowe potrzebne do kompilowania
programСw korzystaj╠cych z biblioteki Xft.

%package xft-static
Summary:	X Font Rendering static library
Summary(pl):	Statyczna biblioteka do renderowania fontСw
Group:		X11/Development/Libraries
...
Fontconfig jest biblioteka przeznaczon╠ do lokalizowania fontСw w
...
Summary(pl):	Pliki nagЁСwkowe dla biblioteki do konfigurowania fontСw
...
Ten pakiet zawiera pliki nagЁСwkowe potrzebne do kompilowania
programСw korzystaj╠cych z biblioteki fontconfig.

%package fontconfig-static
Summary:	Font configuration and customization static library
Summary(pl):	Statyczna biblioteka do konfigurowania fontСw
Group:		Development/Libraries
...
Pakiet zawiera pliki nagЁСwkowe i dokumenetacjЙ dla rozszerzenia
X render. Biblioteka i implementacja serwera znajduj╠ siЙ w osobnym
pakiecie.
...
Summary(pl):	Pliki nagЁСwkowe rozszerzenia X Render
Group:		X11/Libraries
...
Pliki nagЁСwkowe biblioteki X render.
...
Summary(pl):	Pliki nagЁСwkowe biblioteki X cursor
Group:		X11/Libraries/Development
Requires:	%{name}-devel = %{version}
...
Pliki nagЁСwkowe biblioteki X cursor.
...
Podstawowe pliki nagЁСwkowe (tylko gl?.h) OpenGL dla systemu X11R6.
...
Summary(pl):	Pliki nagЁСwkowe do tworzenia sterownikСw/rozszerzeЯ X serwera XFree86
Group:		X11/Development/Libraries
Requires:	%{name}-devel = %{version}
...
Pakiet setup zawiera narzЙdzia do konfiguracji XFree86. Pozwala na
...
Je╤li u©ywasz XСw powiniene╤ zainstalowaФ XFree86-tools. BЙdziesz
rСwnie© musiaЁ zainstalowaФ pakiet XFree86, pakiet odpowiadaj╠cy
...
klienci X, bЙdziesz musiaЁ zainstalowaФ rСwnie© XFree86-devel.
