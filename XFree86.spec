...
Summary(pl.UTF-8):   Wsparcie OpenGL dla systemu X11R6
Group:		X11/Libraries
Requires:	%{name}-libs = %{epoch}:%{version}-%{release}
...
%description Xserver -l de.UTF-8
...
%description Xserver -l fr.UTF-8
...
Jest to podstawowy Xserwer wyświetlający obraz na karcie graficznej.
...
%description Xserver -l tr.UTF-8
...
Aynı zamanda Diamond Speedstar, Orchid Kelvins, STB Nitros / Horizons,
Genoa 8500VL, çoğu Actix kartları, Spider VLB Plus gibi kartlar ve
birçok diğer kart ile de çalışır. Herhangi bir sorun yaşarsanız bu
sunucuyu deneyin.
...
i Xaw. Więcej informacji nt. pisania programów przy użyciu tych
bibliotek możesz znaleźć w książkach wydawnictwa O'Reilly and
...
Summary(pl.UTF-8):   Sterownik do framebuffera DRI na sparc
Group:		X11/XFree86
Requires:	%{name}-Xserver = %{epoch}:%{version}-%{release}
Requires:	%{name}-modules = %{epoch}:%{version}-%{release}
...
%description driver-ffb -l pl.UTF-8
...
%description -n imake -l pl.UTF-8
...
%description -n sessreg -l pl.UTF-8
...
%description -n twm -l ru.UTF-8
Простой компактний оконный менеджер.

%description -n twm -l uk.UTF-8
...
%description -n xauth -l pl.UTF-8
Program xauth służy do edycji i wyświetlania informacji
autoryzacyjnych używanych przy łączeniu z Xserwerem. Ten program
...
%description -n xdm -l ru.UTF-8
Менеджер дисплея X.

%description -n xdm -l uk.UTF-8
Менеджер дисплею X.
